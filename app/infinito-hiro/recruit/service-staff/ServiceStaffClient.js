...
export default function ServiceStaffClient() {
  const [lang, setLang] = useState("ja");
  const toggle = () => setLang((l) => l === "ja" ? "en" : "ja");

  const job = JOBS.find(j => j.id === "service-staff");
  const otherJobs = JOBS.filter(j => j.id !== "service-staff");
      ...
      <main className="ih-stack ih-job-detail">
        ...
        <section className="ih-job-section ih-apply-section">
          ...
          <div className="ih-apply-buttons">
            <a href={`mailto:${company.email}?subject=【求人応募】サービススタッフ`} className="ih-apply-button ih-apply-email">
              {t.applyByEmail}
